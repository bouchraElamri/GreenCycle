...
export default function MobileFiltersPanel({
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/30 md:hidden">
      <div
        ref={mobilePanelRef}
        className="absolute  top-36 left-[5%] h-[70%] w-[90%]  overflow-y-auto bg-white-intense p-4 shadow-xl rounded-xl"
      >
        <div ref={sortRef} className="relative w-full mb-4">
          <button
            ...
            className="flex w-full bg-green-dark font-nexa text-xs text-white-intense px-4 
            py-2 rounded-full justify-between items-center h-9"
          >
            {selectedSort}
          ...
          {openSort && (
            <div className="absolute mt-2  bg-white-intense font-nexa rounded-2xl shadow-lg overflow-hidden z-20">
              {options.map((opt) => (
                <div
                  key={opt.label}
                  onClick={() => {
